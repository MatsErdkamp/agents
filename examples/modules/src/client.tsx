...
let sessionId = localStorage.getItem("modules-example-session");
if (!sessionId) {
  sessionId = crypto.randomUUID().slice(0, 8);
  localStorage.setItem("modules-example-session", sessionId);
}
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-4">
          <div className="min-w-0">
            <div className="flex items-center gap-3">
              ...
              <Badge variant="secondary">@cloudflare/think/modules</Badge>
            </div>
            <p className="mt-1 text-sm text-kumo-subtle">
              A focused example of fluent signatures, `Predict`, composed
      ...
      <footer className="px-5 py-4 border-t border-kumo-line">
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-3">
          <Text size="xs" variant="secondary">
            Built with `Think`, `Module`, `Predict`, and the default AI SDK
            adapter.
          </Text>
          <PoweredByCloudflare />
        </div>
