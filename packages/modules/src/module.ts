...
type ResolvedModuleConfig = {
  ...
  inputFieldDescriptions: Readonly<Record<string, string>>;
  inputFieldDescriptionsVersion: string;
  outputFieldDescriptions: Readonly<Record<string, string>>;
  outputFieldDescriptionsVersion: string;
};
...
export abstract class Module<I extends z.ZodTypeAny, O extends z.ZodTypeAny> {
  ...
  async invoke(
    ...
    const resolved = await resolveModuleConfig(
      context,
      modulePath,
      this.signature
    );
    ...
    const trace: ModuleTrace = {
      ...
      modelId: null,
      adapterName: null,
      instructionVersion: resolved.instructionVersion,
      inputFieldDescriptionsVersion: resolved.inputFieldDescriptionsVersion,
      outputFieldDescriptionsVersion: resolved.outputFieldDescriptionsVersion,
  ...
  public getModuleEntries(): Array<{ path: string; module: AnyModule }> {
    const entries: Array<{ path: string; module: AnyModule }> = [];
    this.collectModuleEntries(entries);
    return entries;
  }
  ...
  private collectModuleEntries(
    entries: Array<{ path: string; module: AnyModule }>
  ) {
    entries.push({ path: this.resolvePath(), module: this });

    for (const child of this.#children.values()) {
      child.collectModuleEntries(entries);
    }
  }
}
...
>(
  ...
  const baseConfig: ResolvedModuleConfig = {
    instructions: signature.instructions,
    instructionVersion: "default",
    inputFieldDescriptions: signature.inputFieldDescriptions,
    inputFieldDescriptionsVersion: "default",
    outputFieldDescriptions: signature.outputFieldDescriptions,
...
function parseArtifactContent<T>(artifact: ModuleArtifact | null): T | null {
  if (!artifact) {
    return null;
  }
